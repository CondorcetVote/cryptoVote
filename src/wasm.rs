...
///
/// `vote` is taken as `&[u8]` so JavaScript can pass either a
/// `Uint8Array` or anything else `wasm_bindgen` knows how to coerce.
#[wasm_bindgen]
pub fn sign_vote_wasm(
    ...
    let proof = crate::sign_vote(&sk, vote, &ring).map_err(err_to_js)?;
...
pub fn verify_vote_wasm(
    ...
    crate::verify_vote(vote, &signature, &key_image, &ring)
}
