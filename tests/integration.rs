...
fn full_flow_round_trip() {
    ...
    let proof = sign_vote(&sk, ballot, &ring).expect("sign");
...
fn serialised_proof_round_trip() {
    ...
    let proof = sign_vote(&sk, ballot, &ring).unwrap();
    ...
    assert!(verify_vote(ballot, &sig, &tag, &ring));
}

#[test]
fn host_can_detect_double_vote_via_tag() {
    // Two different ballots, same voter → same key image. This is the
    // "deterministic tag" property the anti-double-vote contract relies
    // on.
...
fn anonymity_is_indistinguishable_by_index() {
    ...
    let p_a = sign_vote(&voter_a.secret_key, b"yes", &ring).unwrap();
    let p_b = sign_vote(&voter_b.secret_key, b"yes", &ring).unwrap();
    let p_c = sign_vote(&voter_c.secret_key, b"yes", &ring).unwrap();
